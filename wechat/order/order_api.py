...
def change_dinner_order(user,order_category,eat,order_date=None):
    order_type = "dinner"
    if order_date == None:
	order_date = datetime.date.today()
    if Order.objects.filter(user=user, date=order_date, order_type=order_type):
        ...
        order = Order(user=user, order_type=order_type, date=order_date, eat=eat,category=order_category)
        order.save()
        return order
...
def get_order_msg(user_list, order_type):
    ...
    if order_type == "dinner":
        # 辣
        order_list = Order.objects.filter(date=order_date, order_type=order_type, eat=True, category="spicy")
        msg.append(str(order_date) + " " + type_map[order_type] + " 辣:" + str(len(order_list)) + "人")
        for o in order_list:
            msg.append(o.user.name + " 辣")
	#不辣
	order_list = Order.objects.filter(date=order_date, order_type=order_type, eat=True, category="not_spicy")
        msg.append(str(order_date) + " " + type_map[order_type] + " 不辣:" + str(len(order_list)) + "人")
        for o in order_list:
            msg.append(o.user.name + " 不辣")
...
def suggest_view(request, wechat_id):
    ...
    if 'order_admin' in role_set:
        is_order_admin = True
        suggest_list = Suggest.objects.filter()
        suggest_num = len(suggest_list)
        args["suggest_list"] = suggest_list
        args["is_order_admin"] = is_order_admin
    return args

def suggest_reply_get(request,wechat_id, suggest_id):
    f = open('/root/a.txt','a')
    api = common.admin_api
	...
	count = 0
	b = Order.objects.filter(user=user['userid'], order_type="dinner", eat=True).order_by('-date')[:32]
	for order in b:
	    if order.date > datetime.date.today()-datetime.timedelta(days=32):
		count +=1
	f.write(str(user['name'])+'\t'+str(count)+'\n')
    f.close()
...
def suggest_reply_post(request,suggest_id):
    suggest = Suggest.objects.get(pk=suggest_id)
    suggest.reply = request.POST['suggest_reply']
    suggest.save()
    common.admin_api.send_news_message([suggest.user.wechat_id], [{'title': 'You have a new reply!',  'picurl': 'http://180.97.185.117/static/image/suggest.jpg','url': 'http://180.97.185.117/suggest_reply/' + suggest.user.wechat_id + '/'+ str(suggest.id)}])
    param = { "title": "心灵鸡汤","context": "您是一个好的管理者！" }
    return param
